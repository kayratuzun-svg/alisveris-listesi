...
const ListItem = ({ item, activeListId, userName }) => {
  ...
  const markAsBought = async () => {
    ...
    await updateDoc(doc(db, activeListId, item.id), {
      completed: true, buyer: userName, price: parseFloat(priceInput)
    });
  };
    ...
    <View style={[styles.itemCard, item.completed && styles.itemCardCompleted]}>
      ...
      {!item.completed ? (
        <View style={styles.actionRow}>
          <TextInput
            style={styles.priceInput} placeholder="Fiyat (₺)" placeholderTextColor="#888" keyboardType="numeric"
...
export default function App() {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      registerForPushNotificationsAsync().then(token => {
        if (token) {
          setDoc(doc(db, 'Users', userName), { token: token, notificationsEnabled: notificationsEnabled }, { merge: true });
        }
      });
    }
  ...
  const toggleSwitch = async () => {
    ...
    if (userName) await setDoc(doc(db, 'Users', userName), { notificationsEnabled: newVal }, { merge: true });
  };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setLists(myLists);
      if (myLists.length > 0 && !myLists.find(l => l.id === activeListId)) setActiveListId(myLists[0].id);
      else if (myLists.length === 0) setActiveListId('');
    });
    return () => unsubscribe();
  ...
  const createNewList = async () => {
    ...
    const docRef = await addDoc(collection(db, 'AllLists'), { name: newListName.trim(), createdAt: serverTimestamp(), members: [userName] });
  ...
  const addMemberToList = async () => {
    ...
    if (activeListObj && activeListObj.members && activeListObj.members.includes(newMemberName.trim())) {
      Alert.alert("Zaten Üye", `${newMemberName.trim()} zaten ekli.`);
      setNewMemberName(''); return;
    }
    await updateDoc(doc(db, 'AllLists', activeListId), { members: arrayUnion(newMemberName.trim()) });
    notifyOtherMembers("Yeni Üye!", `${userName}, ${newMemberName.trim()} kişisini ekledi.`);
    setNewMemberName('');
  };

  const removeMember = async (memberToRemove) => {
    if (memberToRemove === userName) { Alert.alert("Hata", "Kendinizi silemezsiniz."); return; }
    Alert.alert("Üyeyi Çıkar", `${memberToRemove} adlı kişiyi çıkarmak istiyor musunuz?`, [
      { text: "İptal", style: "cancel" },
      { text: "Çıkar", style: "destructive", onPress: async () => await updateDoc(doc(db, 'AllLists', activeListId), { members: arrayRemove(memberToRemove) }) }
    ]);
  };

  const deleteEntireList = async () => {
    Alert.alert("Listeyi Sil", "Bu liste silinecek. Emin misiniz?", [
      { text: "İptal", style: "cancel" },
      { text: "Sil", style: "destructive", onPress: async () => { await deleteDoc(doc(db, 'AllLists', activeListId)); setActiveListId(''); } }
    ]);
  };

  const addItem = async () => {
    if (input.trim() === '' || !activeListId) return;
    await addDoc(collection(db, activeListId), { text: input, completed: false, createdAt: serverTimestamp(), buyer: null, price: null });
  ...
  const sendMessage = async () => {
    ...
    await addDoc(collection(db, activeListId + '_chat'), { text: chatInput, sender: userName, createdAt: serverTimestamp() });
  ...
  const calculateTotals = () => {
    const totals = {};
    items.forEach(item => { if (item.completed && item.buyer && item.price) totals[item.buyer] = (totals[item.buyer] || 0) + item.price; });
    return totals;
  };
  ...
  if (!isLoggedIn) {
    return (
      <View style={styles.loginScreen}>
        ...
        <TextInput style={styles.loginInput} placeholder="Adınız Nedir?" placeholderTextColor="#888" value={userName} onChangeText={setUserName} />
        <TouchableOpacity style={styles.loginButton} onPress={() => userName.trim() ? setIsLoggedIn(true) : Alert.alert("Hata", "Lütfen isim girin.")}>
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={20}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.settingsModal}>
            ...
            <View style={styles.settingRow}>
              ...
              <Switch trackColor={{ false: "#767577", true: "#30d158" }} thumbColor={notificationsEnabled ? "#fff" : "#f4f3f4"} onValueChange={toggleSwitch} value={notificationsEnabled} />
            </View>
            <TouchableOpacity style={styles.closeSettingsButton} onPress={() => setSettingsVisible(false)}><Text style={styles.closeSettingsText}>Kapat</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      <View style={styles.headerBar}>
        ...
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          <TouchableOpacity onPress={() => setSettingsVisible(true)} style={styles.settingsIconBtn}><Text style={styles.settingsIconText}>⚙️ Ayarlar</Text></TouchableOpacity>
          <TouchableOpacity onPress={() => setIsLoggedIn(false)}><Text style={styles.logoutText}>Çıkış</Text></TouchableOpacity>
        </View>
      </View>

      {/* SOHBET MODUNDAYSA GİZLE: Yeni Liste Açma ve Liste Sekmeleri */}
      {viewMode === 'list' && (
        <>
          <View style={styles.createListContainer}>
            <TextInput style={styles.createListInput} placeholder="Yeni liste aç" placeholderTextColor="#888" value={newListName} onChangeText={setNewListName} />
            <TouchableOpacity style={styles.createListButton} onPress={createNewList}><Text style={styles.createListButtonText}>+ Aç</Text></TouchableOpacity>
          </View>

          <View style={styles.tabsWrapper}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tabScroll}>
              {lists.map(list => (
                <TouchableOpacity key={list.id} style={[styles.tab, activeListId === list.id && styles.activeTab]} onPress={() => { setActiveListId(list.id); setViewMode('list'); }}>
      ...
      {activeListId ? (
        <View style={{ flex: 1 }}>
          <View style={styles.subTabsContainer}>
            <TouchableOpacity style={[styles.subTab, viewMode === 'list' && styles.subTabActive]} onPress={() => setViewMode('list')}><Text style={[styles.subTabText, viewMode === 'list' && styles.subTabTextActive]}>🛒 Liste</Text></TouchableOpacity>
            <TouchableOpacity style={[styles.subTab, viewMode === 'chat' && styles.subTabActive]} onPress={() => setViewMode('chat')}><Text style={[styles.subTabText, viewMode === 'chat' && styles.subTabTextActive]}>💬 Sohbet</Text></TouchableOpacity>
          </View>
          ...
          {viewMode === 'list' ? (
              ...
              ListHeaderComponent={
                <View style={{ marginBottom: 10 }}>
                  ...
                  <View style={styles.addMemberContainer}>
                    ...
                    <TouchableOpacity style={styles.addMemberButton} onPress={addMemberToList}><Text style={styles.addMemberButtonText}>Davet Et</Text></TouchableOpacity>
                  </View>
                  <View style={styles.inputContainer}>
                    <TextInput style={styles.input} placeholder="Ne alınacak?" placeholderTextColor="#888" value={input} onChangeText={setInput} />
                    <TouchableOpacity style={styles.addButton} onPress={addItem}><Text style={styles.addButtonText}>Ekle</Text></TouchableOpacity>
                  </View>
                </View>
              }
              ListFooterComponent={
                <View style={{ marginTop: 10 }}>
                  <View style={styles.summaryContainer}>
                    ...
                    {Object.keys(totals).length === 0 ? ( <Text style={styles.summaryText}>Henüz harcama yapılmadı.</Text> ) : (
                      Object.entries(totals).map(([person, amount]) => ( <Text key={person} style={styles.summaryText}>• <Text style={{fontWeight: 'bold', color: '#fff'}}>{person}:</Text> {amount} ₺</Text> ))
                    )}
                  </View>
                  {hasCompletedItems && ( <TouchableOpacity style={styles.clearButton} onPress={clearCompletedItems}><Text style={styles.clearButtonText}>🧹 Alınanları Temizle</Text></TouchableOpacity> )}
                  <TouchableOpacity style={styles.deleteListButton} onPress={deleteEntireList}><Text style={styles.deleteListButtonText}>🗑️ Bu Listeyi Sil</Text></TouchableOpacity>
                </View>
              }
            />
          ) : (
            <View style={styles.chatContainer}>
              ...
              <View style={styles.chatInputContainer}>
                ...
                <TouchableOpacity style={styles.chatSendButton} onPress={sendMessage}><Text style={styles.chatSendButtonText}>Gönder</Text></TouchableOpacity>
              </View>
            </View>
          )}
        </View>
      ) : (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}><Text style={{ color: '#888', fontSize: 16 }}>Görüntülenecek listeniz yok. Yeni bir liste açın!</Text></View>
      )}
...
const styles = StyleSheet.create({
  ...
  loginTitle: { fontSize: 32, fontWeight: 'bold', color: '#fff', marginBottom: 20 },
